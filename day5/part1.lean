...
def solve (input: return_type_of parse) :=
  ...
  Id.run do
    ...
    repeat
      accessible := []
      for hi: i in [0:grid.size] do
        for hj: j in [0:grid[i].size] do
          if grid[i][j] == 0 then continue else
          let mut neighbours := 0
          for neighbour_i in [(i-1):(i+2)] do
            for neighbour_j in [(j-1):(j+2)] do
              if neighbour_i == i ∧ neighbour_j == j then continue else
              let neighbour := index neighbour_i neighbour_j grid
              neighbours := neighbours + neighbour
          if neighbours < 4 then accessible := (i,j) :: accessible else continue

      for (i,j) in accessible do
        grid := grid.modify i (·.set! j 0)
