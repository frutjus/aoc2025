...
def parse (input: String) :=
  input.lines.map (λl =>
    let bits := l.splitOn " "
    let (_, rest) := bits.head_and_tail
    let (raw_buttons, goal) := rest.map (λrb: String =>
        rb.lipo.splitOn ","
        |> Functor.map String.toNat!)
        |> List.init_and_last
    let buttons := raw_buttons.map (λbs: List Nat => (List.range goal.length).map (if bs.contains · then 1 else 0))
    (goal, buttons))

abbrev Joltage := List Nat
partial def solve1 (goal: Joltage) (buttons: List Joltage): StateM (Std.HashMap (Joltage × List Joltage) (Option Nat)) (Option Nat) := do
  if goal.all (· == 0) then
    pure (some 0)
  else if h: buttons == [] then
    pure none
  else if let some answer := (← get)[(goal, buttons)]? then
    pure answer
  else do
    let button :: buttons' := buttons
    let use_the_button ← do
      if (goal.zipWith (λg b => g == 0 && b != 0) button).any id then
        pure none
      else do
        let goal': Joltage := goal.zipWith (· - ·) button
        let partial_solution ← solve1 goal' buttons
        pure $ (· + 1) <$> partial_solution
    let don't_use_the_button ← do
      solve1 goal buttons'
    let solution := match use_the_button, don't_use_the_button with
      | none, none => none
      | some s, none => some s
      | none, some s' => some s'
      | some s, some s' => some (min s s')
    modify (·.insert (goal,buttons) solution)
    pure solution

abbrev Parms := Unit
def solve (input: return_type_of parse) (_: Parms) :=
  input.map (λ(g,b) => ((solve1 g b).run ∅).fst)
  |> List.foldl (λ | s, (some c) => s + c | s, none => s) 0
...
--solution run time is too long for full input
